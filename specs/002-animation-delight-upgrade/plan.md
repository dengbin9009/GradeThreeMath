...
**Scale/Scope**: 39 个模块、至少 39 组 asset manifest、5 个保留基准模块、4 个第二阶段样板、后续 30 个批量精修模块
...
3. image2 生成资产进入 `apps/web/src/assets/module-frames/<module>/` 或后续统一资产目录。
4. 每个资产记录对应提示词、角色、透明度、可重复渲染和 fallback。
5. 原整图资产可作为参考或 fallback，但不可作为动态数量的唯一表现。
