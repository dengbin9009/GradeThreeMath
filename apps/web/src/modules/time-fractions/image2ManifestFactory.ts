...
export function createTimeFractionImage2Manifest(config: {
  moduleId: Extract<ModuleId, "M22" | "M23" | "M24" | "M25" | "M26" | "M27">;
  promptId: string;
  theme: string;
  prompt: string;
  outputIntent: string;
  ...
  return {
    ...
    version: 1,
    styleGuide: `${config.theme}，明亮教具感，大舞台背景、可动对象、等分/时间量线和反馈道具分层。`,
    prompts: [
      {
        id: config.promptId,
