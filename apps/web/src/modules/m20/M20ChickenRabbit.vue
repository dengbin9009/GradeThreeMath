...
<template>
  <div class="animal-lab">
    ...
    <div class="leg-line" :class="{ visible: pose !== 'standing' }">剩下 {{ remainingLegs }} 条腿，每只兔贡献 2 条</div>
    <div class="controls">
...
<style scoped>
.animal-lab { min-height: 100%; display: grid; grid-template-rows: auto 1fr auto auto; background: #e9f7ef; }.chalkboard { min-height: 76px; display: flex; align-items: center; justify-content: center; flex-wrap: wrap; gap: 16px; padding: 12px 20px; color: white; background: #24553d; }.chalkboard strong { font-size: 20px; }.chalkboard b { color: #ffe082; }.animals { min-height: 330px; padding: 24px; display: flex; flex-wrap: wrap; align-content: center; justify-content: center; gap: 10px; background-image: linear-gradient(transparent 82%, #b9d9bb 82%); }.animals img { width: clamp(62px, 8vw, 105px); height: clamp(82px, 11vw, 135px); object-fit: contain; }.leg-line { text-align: center; min-height: 44px; padding: 10px; color: var(--color-muted); background: white; }.leg-line.visible { color: var(--color-primary); font-weight: 900; }.controls { display: grid; grid-template-columns: repeat(4, 1fr); border-top: 1px solid var(--color-line); }.controls button { border: 0; border-right: 1px solid var(--color-line); background: white; font-weight: 800; }.controls button:nth-child(3) { color: white; background: var(--color-primary); }
